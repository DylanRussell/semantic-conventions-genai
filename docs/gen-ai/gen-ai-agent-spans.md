...
An inference call may be initiated or wrapped by various libraries in the call stack—including prompt optimization libraries (e.g. DSPy), gateways and proxies (e.g. Portkey), agent frameworks, or underlying provider client libraries. The library that initially creates the inference span puts it in the context under `opentelemetry.genai.inference_span`. Any library that is not certain whether it is at the top of the call stack SHOULD check the context for an existing inference span before creating a new one to avoid duplicate spans and double-counted metrics.
